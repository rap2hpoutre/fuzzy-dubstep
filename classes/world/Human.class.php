<?php
require(BASEDIR . 'classes/world/LivingThing.class.php');
require(BASEDIR . 'classes/world/Traits.class.php');

class RPGHuman extends RPGLivingThing {
...
class RPGHumanBuilder {
	...
	public function build() {
		...
		// Traits
		$human->traits = RPGTraitsBuilder::build($human->gender);
		// Téléphone
		$human->phone = '06 ' . (string)mt_rand(0,9) . (string)mt_rand(0,9) . ' ' .
								(string)mt_rand(0,9) . (string)mt_rand(0,9) . ' ' .
								(string)mt_rand(0,9) . (string)mt_rand(0,9) . ' ' .
								(string)mt_rand(0,9) . (string)mt_rand(0,9);
	...
	private function _initNames() {
		self::$ffnames = file(dirname(__FILE__) . '/' . 'ffname.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
		self::$mfnames = file(dirname(__FILE__) . '/' . 'mfname.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
		self::$lnames = file(dirname(__FILE__) . '/' . 'lname.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
